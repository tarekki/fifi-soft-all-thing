...
const getRoleLabel = (role: UserRole, t: any) => {
  const labels = {
    ...
    content_manager: t.admin.users.role.content_manager,
    order_manager: t.admin.users.role.order_manager,
    support: t.admin.users.role.support,
  }
...
const getRoleIcon = (role: UserRole) => {
  const icons = {
    ...
    vendor: Icons.store,
    admin: Icons.shield,
    content_manager: Icons.edit,
    order_manager: Icons.orders,
    support: Icons.users,
  }
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-4">
        ...
        >
          ...
          <option value="content_manager">{t.admin.users.role.content_manager}</option>
          <option value="order_manager">{t.admin.users.role.order_manager}</option>
              ...
              <tbody className="divide-y divide-historical-gold/5 dark:divide-gray-700/50 transition-colors duration-300">
                {users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${user.is_active
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${user.is_active ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'
                      }`}>
                      {user.is_active ? t.admin.users.active : t.admin.users.inactive}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${user.is_staff ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
                      }`}>
